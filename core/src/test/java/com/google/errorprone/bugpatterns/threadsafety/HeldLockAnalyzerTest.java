...
public class HeldLockAnalyzerTest {
  ...
  @Before
  public void setUp() {
    compilationHelper = new CompilationTestHelper(new GuardedByLockSetAnalyzer());
  }
  ...
  public void testTwoInstances() throws Exception {
    compilationHelper.assertCompileFailsWithMessages(
        CompilationTestHelper.forSourceLines(
            ...
            "    try {",
            "    // BUG: Diagnostic contains:",
            "    // [(LOCAL_VARIABLE lock2), (SELECT (THIS_LITERAL threadsafety.Test.Test) lock)]",
            "    x++;",
  ...
  public void testSynchronizedMethod() throws Exception {
    compilationHelper.assertCompileFailsWithMessages(
        CompilationTestHelper.forSourceLines(
            ...
            "  int x;",
            "  synchronized void m() {",
            "    // BUG: Diagnostic contains:  [(THIS_LITERAL threadsafety.Test.Test)]",
            "    x++;",
            "  }",
            "}"
        )
  ...
  public void testSynchronizedField() throws Exception {
    compilationHelper.assertCompileFailsWithMessages(
        CompilationTestHelper.forSourceLines(
            ...
            "      // BUG: Diagnostic contains:",
            "      // [(SELECT (SELECT (THIS_LITERAL threadsafety.Test.Test) mu) lock)]",
            "      x++;",
            "    }",
            "  }",
            "}"
  ...
  public void testLocked() throws Exception {
    compilationHelper.assertCompileFailsWithMessages(
        CompilationTestHelper.forSourceLines(
            ...
            "    try {",
            "      // BUG: Diagnostic contains:",
            "      // [(SELECT (THIS_LITERAL threadsafety.Test.Test) mu)]",
            "      x++;",
            "    } finally {",
            "      mu.unlock();",
            "    }",
