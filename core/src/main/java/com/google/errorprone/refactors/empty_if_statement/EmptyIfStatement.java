 ...
 * (foo == 10);
 *
 * It attempts to match javac's -Xlint:empty warning behavior, which can be found in
...
public class EmptyIfStatement extends RefactoringMatcher<EmptyStatementTree> {
  ...
  /**
   * We suggest different fixes depending on what follows the parent if statement. If there is no
   * statement following the if, then suggest deleting the whole if statement. If the next statement
   * is a block, then suggest deleting the empty then part of the if.  If the next statement is not
   * a block, then also suggest deleting the empty then part of the if.
   */
  @Override
  public Refactor refactor(EmptyStatementTree tree, VisitorState state) {
    ...
    if (nextStmtIsNull) {
      ...
      return new Refactor(parent.getThenStatement(), refactorMessage, fix);
    }
  }
