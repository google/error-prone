 ...
 *
 */
public class OrderingFromChecker extends ErrorChecker<MethodInvocationTree> {
  @Override
  ...
  @Override
  public AstError produceError(MethodInvocationTree t,
      VisitorState state) {
    ...
    SuggestedFix fix =
        SuggestedFix.replace(
            getPosition(identifier), "Ordering<" + identifier.getTypeArguments() + ">")
        .replace(getPosition(t).start, getPosition(invocation).start, "")
        .replace(getPosition(t).end - 1, getPosition(t).end, "");
    
    return new AstError(t, "Call to Guava's Ordering.from() taking an anonymous inner "
        + "subclass of Comparator<T>; suggest using new Ordering intead.", fix);
  }
