...
@BugPattern(name = "IncrementDecrementVolatile",
    summary = "Incrementing or decrementing a volatile variable is non-atomic",
    explanation = "The volatile modifier ensures that updates to a variable are propagated "
        + "predictably to other threads.  A read of a volatile variable always returns the most "
        + "recent write by any thread.\\n"
        + "However, this does not mean that increments or decrements of a volatile variable are "
        + "atomic.  If you increment or decrement a volatile variable, you are actually doing "
        + "(1) a read of the variable, (2) an increment or decrement of a local copy, and "
        + "(3) a write back to the variable. Each step is atomic individually, but the whole "
        + "sequence is not, and it will cause a race condition if two threads try to increment "
        + "or decrement a volatile variable at the same time.\\n"
        + "If you intended for this increment or decrement to be atomic, you should should either "
        + "use an AtomicInteger instead of a volatile int, or wrap all increment or decrement "
        + "operations on this variable in a synchronized block.",
    category = JDK, severity = WARNING, maturity = MATURE)
public class IncrementDecrementVolatile extends BugChecker
