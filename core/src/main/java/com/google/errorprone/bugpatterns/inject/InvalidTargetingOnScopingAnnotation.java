...
@BugPattern(
  ...
  explanation =
      "Scoping annotations are only appropriate for provision and therefore are only "
...
public class InvalidTargetingOnScopingAnnotation extends BugChecker implements ClassTreeMatcher {
  ...
  public Description describe(ClassTree classTree, VisitorState state) {
    Compound target =
        ASTHelpers.getSymbol(classTree).attribute(state.getSymbolFromString(TARGET_ANNOTATION));
    if (target == null) {
