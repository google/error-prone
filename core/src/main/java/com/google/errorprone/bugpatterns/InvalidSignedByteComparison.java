 ...
 */
@BugPattern(name = "SignedByteComparison",
    summary = "Bad comparison of signed byte",
    explanation = "Signed bytes can only have a value in the range -128 to 127. Comparing "
 + " a signed byte with a value outside that range is vacuous and likely to be incorrect.",
    category = JDK, severity = ERROR, maturity = EXPERIMENTAL)
public class InvalidSignedByteComparison extends DescribingMatcher<BinaryTree> {

    private static  boolean match(ExpressionTree leftOperand,   ExpressionTree rightOperand,  VisitorState state) {
        Type byteType = state.getSymtab().byteType; 
   ...
   */
  private static final Matcher<BinaryTree> BAD_SIGNED_BYTE_COMPARISON = new Matcher<BinaryTree>() {
  ...
  private String fix(ExpressionTree leftOperand, Kind kind,
      ...
      return leftOperand.toString() + (kind == Tree.Kind.EQUAL_TO ? " == " : " != ")
               + Byte.toString(byteValue);
}

public static class Scanner extends com.google.errorprone.Scanner {
    private InvalidSignedByteComparison matcher = new InvalidSignedByteComparison();
