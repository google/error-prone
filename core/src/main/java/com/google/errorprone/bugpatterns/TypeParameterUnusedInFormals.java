...
@BugPattern(name = "TypeParameterUnusedInFormals",
    ...
    + "<p>First, operations on the type will be unchecked after the type parameter is erased."
    + " For example:"
    + "<p><pre>"
    ...
    + "}\n"
    + "</pre>"
    + "<p>The 'doCast' method would be better implemented as:"
    + "<p><pre>"
    + "static <T> T doCast(Class<T> clazz, Object o) {\n"
    + "  return clazz.cast(o); // has the expected behaviour\n"
    + "}\n"
    + "</pre>"
    + "<p>Second, this pattern causes unsafe casts to occur at invocations of the method. Consider"
    ...
    + "</pre>"
    + "<p>Finally, relying on the type parameter to be inferred can have surprising results, and"
