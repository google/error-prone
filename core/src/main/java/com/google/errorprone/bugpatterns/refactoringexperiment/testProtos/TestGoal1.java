...
import com.google.errorprone.bugpatterns.refactoringexperiment.analysis.Analyzer;
...
public class TestGoal1 {
    ...
    public void test_OnlyMethodInvocation_AllLambda() throws Exception {
        List<Refactorable> output = Analyzer.induceAndMap("goal1_mthd_invc_lambda_only/").stream().collect(Collectors.toList());
    ...
    public void test_AssignmentAndInvocationFail() throws Exception {
        List<Refactorable> output = Analyzer.induceAndMap("goal1_assignment_lambda_mthd_invc/").stream().collect(Collectors.toList());
