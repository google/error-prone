...
public class DoNotStoreChecker extends BugChecker
  ...
  private Description match(ExpressionTree tree, VisitorState state) {
    ...
    String doNotStore = ASTHelpers.getAnnotation(tree, DoNotStore.class).value();
    StringBuilder message =
        new StringBuilder("The value returned from this method should not be stored");
    if (doNotStore.isEmpty()) {
      message.append(", see its documentation for details.");
    } else {
      message.append(": ").append(doNotStore);
    }
