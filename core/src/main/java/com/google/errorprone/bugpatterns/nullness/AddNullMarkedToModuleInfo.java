...
/** Refactoring to add {@code @NullMarked} annotation to module-info.java files. */
@BugPattern(summary = "Apply @NullMarked to this module", severity = SUGGESTION)
public final class AddNullMarkedToModuleInfo extends BugChecker
    implements CompilationUnitTreeMatcher {
  @Override
  public Description matchCompilationUnit(CompilationUnitTree unit, VisitorState state) {
    if (!isModuleInfo(unit)) {
      return NO_MATCH;
    }
    ...
    return describeMatch(
        ...
        SuggestedFix.builder()
            .prefixWith(module, "@NullMarked ")
            .addImport("org.jspecify.annotations.NullMarked")
            .build());
  }

  private static boolean isModuleInfo(CompilationUnitTree tree) {
