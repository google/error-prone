...
presented by Louis Wasserman at the *Workshop for Refactoring Tools*.
...
You can also use `-XepPatchLocation:IN_PLACE`, and Error Prone will modify the
source files in place, without creating `.patch` files.
...
This refactoring rewrites *expressions* of the form
...
This is called an *expression* template because it rewrites expressions. Let's
look at a *block* template:
...
We have also found Refaster good for expressing code *simplification* patterns
...
impossible-to-compile code you would *want* to write here, `T.class`. There are
...
method. (Note that Refaster will *not* pay attention to how the class is
...
would *not* be matched, because the expression in the `put` refers to `map`,
