...
*   Condition-checking expressions are often repeated multiple times, once in
    ...
    an `if` in the chain that unintentionally has a slightly different condition
    than others may require careful review to spot
...
Note that with the `switch` style, one gets exhaustiveness checking "for free".
That is, if a new `Suit` value were to be added to the `enum`, then the `switch`
would raise a compile-time error, whereas the original chain of `if` statements
would need to be manually detected and edited.
