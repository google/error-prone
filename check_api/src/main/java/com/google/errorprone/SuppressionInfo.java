...
public class SuppressionInfo {
  ...
  static class SuppressionInfoForSymbol {
    private final @Nullable Symbol deltaSymbol;
    private final ImmutableSet<String> deltaSuppressWarningsStrings;
    private final Set<String> usedSuppressWarningsStrings = new HashSet<>();
    private final ImmutableSet<Name> deltaCustomSuppressions;
    private final @Nullable SuppressionInfoForSymbol parent;

    public SuppressionInfoForSymbol(
        @Nullable Symbol deltaSymbol,
        ImmutableSet<String> deltaSuppressWarningsStrings,
        ImmutableSet<Name> deltaCustomSuppressions,
        @Nullable SuppressionInfoForSymbol parent) {
      this.deltaSymbol = deltaSymbol;
      this.deltaSuppressWarningsStrings = deltaSuppressWarningsStrings;
      this.deltaCustomSuppressions = deltaCustomSuppressions;
      this.parent = parent;
    }

    public void markSuppressionStringAsUsed(String suppressionName) {
      if (deltaSuppressWarningsStrings.contains(suppressionName)) {
        usedSuppressWarningsStrings.add(suppressionName);
      } else if (parent != null) {
  ...
  public void warnOnUnusedSuppressions(VisitorState state) {
    ...
    for (String warn : infoForClosestSymbol.deltaSuppressWarningsStrings) {
      if (!infoForClosestSymbol.usedSuppressWarningsStrings.contains(warn)) {
        Tree tree =
            Trees.instance(JavacProcessingEnvironment.instance(state.context))
                .getTree(infoForClosestSymbol.deltaSymbol);
  ...
  public SuppressionInfo withExtendedSuppressions(
    ...
    Set<String> deltaSuppressions = new HashSet<>();
            ...
            if (value.snd
              ...
              for (Attribute suppress : array.values) {
                ...
                if (!suppressWarningsStrings.contains(suppressedWarning)) {
                  ...
                  if (allSuppressWarningsStrings.contains(suppressedWarning)) {
                    deltaSuppressions.add(suppressedWarning);
                  }
                }
              }
            } else {
    ...
    SuppressionInfoForSymbol newInfoForClosestSymbol =
        new SuppressionInfoForSymbol(
            sym,
            ImmutableSet.copyOf(deltaSuppressions),
